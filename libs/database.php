...
final class MD_Database
{
  ...
  {
    if (false === ($this->_conn = mysql_connect($this->_conf['host'], $this->_conf['user'], $this->_conf['passwd'], true))) {
      throw new MD_Db_Exception($this->error(), $this->errno());
    }
...
//    $this->exec('SET SQL_MODE = ""');
  ...
  public function dbSelect($name)
  {
    if (!mysql_select_db($name, $this->_conn)) {
      ...
      $errno = $this->errno();
      $this->disconnect();
      throw new MD_Db_Exception($error, $errno);
    }
  }
  ...
  public function query($sql)
  {
    ...
    if (!$result) {
      throw new MD_Db_Exception($this->error(), $this->errno(), $sql);
    } else {
      return new MD_Db_Result($result);
    }
  }
    ...
    if ($recKey == 'WHERE' || $recKey == 'HAVING') {
      ...
      if (isset($rec[$recKey][0])) {
        ...
        foreach ($rec[$recKey] as $num => $cond) {
          ...
          if (false === strpos($cond['field'], '.') && $cond['field'][0] != '(') {
            $sql .= ' `' . $cond['field'] . '`';
          } else {
            $sql .= ' ' . $cond['field'];
          }
...
final class MD_Db_Exception extends Exception
{
  private $_sql;
