...
class session
{
  ...
  public static function read($sid)
  {
    ...
    $result = $db->query('SELECT s.data as session_data, s.* , u.* FROM users u INNER JOIN sessions s ON u.uid = s.uid WHERE s.sid = "' . $db->escape($sid) .
      '" AND timestamp >= ' . $db->escape(TIMESTAMP - MD_Config::get('session.lifetime', 86400)));
    $user = $result->row();
  ...
  {
    global $db;
    $db->exec('DELETE FROM sessions WHERE timestamp < ' . $db->escape(TIMESTAMP - MD_Config::get('session.lifetime', 10800)));
    $db->disconnect();
    return true;
  }
